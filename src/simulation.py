...
class Simulation:
    ...
    def _do_turn(self) -> list[str]:
        ...
        for drone in self.drones:
            ...
            # we only count drones that are in a zone, if the drone is in
            # traveling we don't count it because it's not in a zone
        ...
        # this turn, we do that because we want to make sure that we don't
        # exceed the max_link_capacity of the connection and we don't move
        # the same drone twice in the same turn
        ...
        # zones and move them to their destination (for restricted zones that
        # take 2 turns to enter)
        for drone in self.drones:
        ...
        # here we sort the active drones based how they are cloase to
        # their goal small first the closest to the goal and the largest
        # last the farthest from the goal, we do that because we want to move
        # the closest drones first so they can reach their destination faster
        # and we can free up space for the other drones to move
        ...
        for drone in active:
            ...
            # drones using this connection in this turn} if the number of
            # drones using this connection is greater than or equal to the
            # max_link_capacity of the connection we skip this drone and move
            # to the next one
            if conn_usage.get(conn_key, 0) >= conn.max_link_capacity:
                continue

            # Check destination zone capacity
            # zone_occ{zone_name: number of drones in this zone} if number
            # of drones in dest zone >= max_drones we skip this drone
            ...
            if next_zone.zone_type == "restricted":
                ...
                # The drone is no longer in any zone — it's on the connection
                # between two zones. Empty string means "not in a zone".
                drone.position = ""
                # we mark this conx as used so that we can check the capacity
                # of the connection in the next turn and we can skip drones
                # that want to use this connection if the capacity is full
                ...
                drone.path_index += 1
                # Add 1 to occupancy of the destination zone. This is
                # important for capacity checks for other drones in this turn.
        ...
        # Live coding: record capacity snapshot for this turn if drones moved  # noqa: E501
        if movements:
